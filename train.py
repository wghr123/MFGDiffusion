...
from Module.maskloss import dilate_or_erode_fixed_random
...
def parse_args():
    ...
    parser.add_argument(
        "--mask_random_difference_loss",
        type=float,
        default=0.1,
        help="mask loss power",
    )
...
def main():
    ...
    # load scheduler,tokenizer, modified models
    ...
    model = MaskFeatureDiffusion(args.pretrained_model_name_or_path)
    ...
    def tokenize_captions(examples, is_train=True):
        captions = []
        for caption in examples["en_text"]:
            if isinstance(caption, str):
                caption = "add smoke " + caption 
                ...
                raise ValueError(
                    f"Caption column `en_text` should contain either strings or lists of strings."
                )
        inputs = tokenizer(
            captions,
    ...
    mask_transforms = transforms.Compose(
        [
            ...
            transforms.RandomHorizontalFlip() if args.random_hflip else transforms.Lambda(lambda x: x),
            transforms.RandomVerticalFlip() if args.random_vflip else transforms.Lambda(lambda x: x),
            transforms.ToTensor(),  
        ]
    )
    ...
    def preprocess_train(examples):
        ...
        examples["masks"] = [mask_transforms(Image.open(mask_path).convert("L")) for mask_path in examples["mask_path"]]
        # examples["RGB_masks"] = [mask_transforms(Image.open(mask_path).convert("L")) for mask_path in examples["mask_path"]]
        examples["masked_images"] = []
        for pixel_values, mask in zip(examples["pixel_values"], examples["masks"]):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                mask = torch.nn.functional.interpolate(
                    batch["masks"],
                    size=(
                        batch["pixel_values"].shape[2] // vae_scale_factor,
                        batch["pixel_values"].shape[3] // vae_scale_factor,
                    ),
                )
                ...
                #print(noisy_latents.shape, mask.shape, masked_image_latents.shape)
                ...
                model_pred = model(
                    latent_model_input, timesteps, encoder_hidden_states, batch["masked_images"], batch["masks"]
                )

                if args.mask_random_difference_loss is None:
                    loss = F.mse_loss(
                        model_pred.float(), target.float(), reduction="mean"
                    )
                    #print("use base loss function")
                else:
                    # use masked_loss to get more consistent boundaries
                    weight = args.mask_random_difference_loss
                    old_loss = F.mse_loss(
                        model_pred.float(), target.float(), reduction="mean"
                    )
                    difference_mask = dilate_or_erode_fixed_random(mask)
                    model_pred_mask = model_pred * (difference_mask < 0.5)
                    target_mask = target * (difference_mask < 0.5)
                    new_loss = F.mse_loss(
                        model_pred_mask.float(), target_mask.float(), reduction="mean"
                    )
                    loss = (1 - weight) * old_loss + weight * new_loss
